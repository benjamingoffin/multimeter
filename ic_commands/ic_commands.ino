...
// INA219 Sensor and LCD screen attached to 
//   I2C (Inter-Integrated Circuit) bus as follows:
...
// SD card attached to 
//   SPI (Serial Peripheral Interface) bus as follows:
...
void loop()
{
  ...
  String dataString = String(dtostrf(current,8,4,buffer)) + "," + dtostrf(voltage,8,6,buffer) + "," + dtostrf(resistance,8,4,buffer);
  //See if file can be written/opened
  File logFile = SD.open("LOG.csv", FILE_WRITE);
  if (logFile) {
